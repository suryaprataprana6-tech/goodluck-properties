...
export async function sendLeadEmailNotification(lead: Lead): Promise<boolean> {
  ...
  const emailBody = `
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #114c33; max-width: 600px; border: 1px solid #d4af37; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
      ...
      <div style="padding: 24px; bg-color: #fcfcfc;">
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 15px; font-size: 13px;">
          ...
          <tr>
            ...
            <td style="padding: 8px 0; color: #718096; font-mono">${lead.submittedAt}</td>
          </tr>
        </table>
      </div>
  ...
  if (!host || !user || !pass) {
    console.log("⚠️ Nodemailer: SMTP_HOST, SMTP_USER, or SMTP_PASS not set in environment variables. Email notification log output below:");
    console.log("📨 Destination Desks:", toEmails);
    console.log("📨 Body details:", emailBody);
    return false;
  }
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      to: toEmails.join(", "),
      subject: `New Property Lead - Goodluck Properties (${lead.name})`,
      html: emailBody,
    });
